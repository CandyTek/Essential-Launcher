		...
		for (int i2 = 0;i2 < this.unfilteredList.size();i2++) {
			...
			for (String str : split) {
				if (!str.isEmpty() && !str.trim()
						.isEmpty() && ((this.showHiddenApps || !applicationModel2.hidden) && applicationModel2.label != null && applicationModel2.className != null && applicationModel2.packageName != null && (applicationModel2.label.toLowerCase(
						this.locale).contains(str) || applicationModel2.className.toLowerCase(this.locale)
						.contains(str) || applicationModel2.packageName.toLowerCase(this.locale).contains(str)))) {
