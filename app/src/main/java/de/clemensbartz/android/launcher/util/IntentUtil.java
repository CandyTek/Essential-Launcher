    ...
    public static boolean isCallable(PackageManager packageManager, Intent intent) {
        if (packageManager != null && intent != null) {
            for (ResolveInfo resolveInfo : packageManager.queryIntentActivities(intent, 65536)) {
                if (resolveInfo.activityInfo.exported && (resolveInfo.activityInfo.permission == null || packageManager.checkPermission(resolveInfo.activityInfo.permission, BuildConfig.APPLICATION_ID) == 0)) {
                    return true;
                }
            }
        }
