...
public class Han2Pin {
    ...
    // 全拼
    private static final String[] pystr = new String[]{"a","ai","an","ang","ao","ba","bai","ban","bang","bao","bei","ben","beng","bi","bian",
    ...
    public static String[] mainPY = pystr;
    /** 不能实例化*/
    private Han2Pin() {}
    ...
    private static int getChsAscii(String chs) {
        ...
        return bytes.length == 1 ? bytes[0] : (256 * bytes[0] + 256 + bytes[1]);
    }
    /** 获取单字的Ascii码 */
    private static int getChsAscii(char chs) {
        byte[] bytes;
        try {
            bytes = String.valueOf(chs).getBytes("GBK");
        }
        catch (Exception e) {
            return 1;
        }
        ...
        if (bytes.length == 1) {
            // ASCII
            return bytes[0] & 0xFF; // 0..255
        } else if (bytes.length == 2) {
            // 关键：让结果落在负数区间，匹配 pyvalue
            return (bytes[0] << 8) + (bytes[1] & 0xFF);
            // 等价写法（也可）:
            // return bytes[0] * 256 + (bytes[1] & 0xFF);
        } else {
            return 1;
        }
    }
    ...
    public static String getSelling(String chs) {
        ...
        for (int i = 0;i < chs.length();i++) {
            ...
            String value = key.getBytes().length >= 2 ? convert(key) : key;
            buffer.append(value);
        }
